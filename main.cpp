...
struct particle
{
	...
	particle(int pS, double R, double T,int G) : popSize(pS), ratio(R), time(T), genomeSize(G)	{
	}
};
...
double calcAvgBlocks(const std::vector<Fish>& Pop)
{
	...
	averageNumBlocks = 1.0 * averageNumBlocks / (2 * Pop.size()); //the number of blocks averaged per (diploid) individual
	return averageNumBlocks;
}


std::vector<double> doSimulation(const particle& Params, int RecomDist, int selfing, double numRecombinations)
{
	...
	for(int t = 1; t < Params.time; ++t)
	{
		...
		for(int i = 0; i < Params.popSize; ++i)
		{
			...
			Fish kid = mate(Pop[index1],Pop[index2], RecomDist,numRecombinations);
...
int main(int argc, const char * argv[]) {
	...
	macstart(argv); //this function checks if the code is executed on OS X, and then changes the working directory to the directory of the executable.
	...
	if(P.refit == 10)
	{
		particle genParams(P.popSize,P.initRatio,P.maxTime, P.genomeSize);
		
		for(int r = 0; r < P.replicates; ++r) {
			std::cout << genParams << "\t" << r << "\n";
			std::vector<double> avgBlocks = doSimulation(genParams, P.recomDist, P.selfing, P.numberRecombinations);
			
			std::ofstream outFile("record.txt",std::ios::app);
			outFile << genParams << "\t" << r << "\t";
			
			for(int t = 0; t < (int)avgBlocks.size(); ++t)
			{
				outFile << avgBlocks[t] << "\t";
			}
			outFile << "\n";
